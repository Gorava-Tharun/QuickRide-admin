...
/// Global Reactive Connectivity Service for QuickRide Admin App.
///
/// Monitors network status, dispatches connectivity change events,
/// handles automatic reconnection synchronization of dashboard & metrics,
/// and supports testable mock offline simulation.
class AdminConnectivityService {
  static final AdminConnectivityService _instance =
      AdminConnectivityService._internal();
  factory AdminConnectivityService() => _instance;
  AdminConnectivityService._internal();
  ...
  void _updateOnlineState(bool newStatus) {
    ...
    debugPrint('[AdminConnectivityService] Network status changed: ${_isOnline ? "ONLINE" : "OFFLINE"}');
  ...
  Future<void> _notifyReconnectionListeners() async {
    debugPrint('[AdminConnectivityService] Internet restored. Synchronizing admin collections...');
    for (final listener in List.of(_reconnectionListeners)) {
      try {
        await listener();
      } catch (e) {
        debugPrint('[AdminConnectivityService] Error in reconnection listener: $e');
      }
    }
  }
